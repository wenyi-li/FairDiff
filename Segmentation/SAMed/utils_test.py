...
def compute_multi_class_auc(preds: np.array, labels: np.array):
    ...
    # auc_scores = [score for score in auc_scores if not np.isnan(score)]
...
def test_single_image(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    with torch.no_grad():
        ...
        softmaxed_prob = torch.softmax(output_masks, dim=1)
        # Assuming a function to compute multi-class AUC exists
        auc_score = compute_multi_class_auc(softmaxed_prob.squeeze().cpu().detach().numpy(), label)
        out = torch.argmax(torch.softmax(output_masks, dim=1), dim=1).squeeze(0)
        out = out.cpu().detach().numpy()
        
        if x != out.shape[0] or y != out.shape[1]:
            print('Resizing prediction to match input dimensions')
            pred = zoom(out, (x / out.shape[0], y / out.shape[1]), order=0)
        else:
            pred = out
    ...
    # Visualize and/or save the segmentation result
    if test_save_path and case:
        ...
        plt.subplot(1, 3, 1)
        plt.imshow(image.squeeze().cpu().numpy()[0], cmap='gray', interpolation='none')
        plt.title('Original Image')
        
        plt.subplot(1, 3, 2)
        plt.imshow(label, cmap='jet', interpolation='none')
        plt.title('True Mask')
        
        plt.subplot(1, 3, 3)
        plt.imshow(prediction, cmap='jet', interpolation='none')
        plt.title('Predicted Mask')
        
        # Save the figure
        plt.savefig(os.path.join(test_save_path, f'{case}_segmentation.png'))
        plt.close()
    elif not test_save_path:
        # If not saving, simply display the images inline (useful for Jupyter Notebooks)
        plt.figure(figsize=(12, 6))
