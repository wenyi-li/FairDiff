...
def trainer_fairseg(args, model, snapshot_path):
    ...
    db_train = FairSeg_dataset(base_dir=args.root_path, attr_label=args.attribute, split="train", args=args, 
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size], \
                                                    center_crop_size=args.center_crop_size, use_normalize=True)]), balanced=False, save_path = snapshot_path)
    ...
    for epoch_num in iterator:
        ...
        if epoch_num > int((max_epoch / 2) + 50) and (epoch_num + 1) % save_interval == 0:
