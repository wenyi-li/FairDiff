...
from datasets.dataset_fairseg_abs import FairSeg_dataset, TestGenerator  # 严格按照list地址读取
...
def inference(args, multimask_output, db_config, model, test_save_path=None, no_of_attr=3):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        metric_i, cdr_dist, auc_score = test_single_image(image, label, model, classes=args.num_classes, multimask_output=multimask_output,
                                                          patch_size=[args.img_size, args.img_size], input_size=[
    ...
    logging.info(
        '--------- Overall Performance for Attribute: {} -----------'.format(args.attribute))
    ...
    for one_attr in range(no_of_attr):
        ...
        logging.info(
            f'{one_attr}-attr overall dice: {np.mean(one_attr_dice_list):.4f}')
        logging.info(
            f'{one_attr}-attr overall hd95: {np.mean(one_attr_hd_list):.4f}')
        logging.info(
            f'{one_attr}-attr overall Jaccard/IoU: {np.mean(one_attr_jc_list):.4f}')
    ...
    logging.info(
        '--------- Cup Performance for Attribute: {} -----------'.format(args.attribute))
    ...
    es_cup_dice = equity_scaled_perf(
        dice_by_attr_cup, cup_overall_dice, no_of_attr)
    es_std_cup_dice = equity_scaled_std_perf(
        dice_by_attr_cup, cup_overall_dice, no_of_attr)
    es_cup_iou = equity_scaled_perf(
        jc_by_attr_cup, cup_overall_jaccard, no_of_attr)
    es_std_cup_iou = equity_scaled_std_perf(
        jc_by_attr_cup, cup_overall_jaccard, no_of_attr)
    ...
    for one_attr in range(no_of_attr):
        ...
        logging.info(
            f'{one_attr}-attr dice for Cup: {np.mean(one_attr_dice_list):.4f}')
        experiment_data[f'{one_attr}-attr dice for Cup'] = np.mean(
            one_attr_dice_list)
        logging.info(
            f'{one_attr}-attr hd95 for Cup: {np.mean(one_attr_hd_list):.4f}')
        logging.info(
            f'{one_attr}-attr Jaccard/IoU for Cup: {np.mean(one_attr_jc_list):.4f}')
        experiment_data[f'{one_attr}-attr IoU for Cup'] = np.mean(
            one_attr_jc_list)
    ...
    logging.info(
        '--------- Rim Performance for Attribute: {} -----------'.format(args.attribute))
    ...
    es_rim_dice = equity_scaled_perf(
        dice_by_attr_rim, rim_overall_dice, no_of_attr)
    es_std_rim_dice = equity_scaled_std_perf(
        dice_by_attr_rim, rim_overall_dice, no_of_attr)
    es_rim_iou = equity_scaled_perf(
        jc_by_attr_rim, rim_overall_jaccard, no_of_attr)
    es_std_rim_iou = equity_scaled_std_perf(
        jc_by_attr_rim, rim_overall_jaccard, no_of_attr)
    ...
    for one_attr in range(no_of_attr):
        ...
        logging.info(
            f'{one_attr}-attr dice for Rim: {np.mean(one_attr_dice_list):.4f}')
        experiment_data[f'{one_attr}-attr dice for Rim'] = np.mean(
            one_attr_dice_list)
        logging.info(
            f'{one_attr}-attr hd95 for Rim: {np.mean(one_attr_hd_list):.4f}')
        logging.info(
            f'{one_attr}-attr Jaccard/IoU for Rim: {np.mean(one_attr_jc_list):.4f}')
        experiment_data[f'{one_attr}-attr IoU for Rim'] = np.mean(
            one_attr_jc_list)
...
# }
    ...
    # append_to_csv('/path/to/your/FairSegDiff/SAMed/equal_scale_out/test_outputs/experiment_results.csv', experiment_data)

    csv_path = args.output
    csv_parent_path = os.path.abspath(os.path.join(csv_path, os.pardir))
    csv_parent_parent_path = os.path.abspath(
        os.path.join(csv_parent_path, os.pardir))
    append_to_csv(os.path.join(csv_parent_parent_path,
                  'experiment_results.csv'), experiment_data)
...
def append_to_csv(file_name, dict_data):
    formatted_dict_data = {key: f'{value:.4f}' if isinstance(
        value, float) else value for key, value in dict_data.items()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str,
                        default='FairSeg', help='Experiment name')
    parser.add_argument('--num_classes', type=int, default=2)
    parser.add_argument(
        '--output', type=str, default='./results/FairSeg_Yan_Loss_regular_gamma1.0_tanh_race/')
    ...
    parser.add_argument('--list_dir', type=str,
                        default='./SAMed/lists/FairSeg_final', help='list dir')
    parser.add_argument('--is_savenii', action='store_true',
                        help='Whether to save results during inference')
    parser.add_argument('--deterministic', type=int, default=1,
                        help='whether use deterministic training')
    parser.add_argument(
        '--ckpt', type=str, default='checkpoints/sam_vit_b_01ec64.pth', help='Pretrained checkpoint')
    ...
    sam, img_embedding_size = sam_model_registry[args.vit_name](image_size=args.img_size,
                                                                num_classes=args.num_classes,
                                                                checkpoint=args.ckpt, pixel_mean=[
                                                                    0, 0, 0],
                                                                pixel_std=[1, 1, 1])
    ...
    inference(args, multimask_output,
              dataset_config[dataset_name], net, test_save_path, no_of_attr)
